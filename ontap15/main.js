...
const showPro = (data) => {
    tbody.innerHTML = data.map((pro, index) => {
        ...
        <td>${pro.gia}</td>
        <td>
            <button data-id="${pro.id}" class="btn-remove">xoa</button>
...
document.querySelector("#btn-add").addEventListener("click", addPronduct);
...
const updateProduct = (id) => {
    ...
    fetch(`http://localhost:3000/pro/${id}`)
        ...
        <br>
        <input type="text" id="gia" value="${data.gia}">
        <br>
